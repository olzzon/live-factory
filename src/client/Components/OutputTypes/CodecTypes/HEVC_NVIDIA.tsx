	...
	useEffect(() => {
		...
		dispatch(
			storeSetFilterParamArr(
				id,
				['{arg3}', '-c:v', 'hevc_nvenc', '-preset', 'llhq', '-b:v', '{arg0}k', '-cbr', 'true', '-zerolatency', 'true', '-pix_fmt', 'yuv420p', '-c:a']
				)
				)
				...
				if (!deinterlaceState) {
					...
					dispatch(storeSetFilterValue(id, 3, {valueArg: ['-vf', 'yadif_cuda']}))
				}
			}, [])
...
export default HevcNvidiaCodecOptions
